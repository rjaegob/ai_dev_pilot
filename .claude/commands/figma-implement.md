---
description: Figma 링크/노드를 React+TS+Tailwind 토큰 기반 컴포넌트로 구현. figma-implementer와 동일한 Clarify→Reuse→Implement→Evaluate 4단계를 메인 루프에서 직접 수행.
argument-hint: <Figma 링크 또는 노드 ID> [출력 파일 경로]
allowed-tools: Read, Write, Edit, Grep, Glob, mcp__figma__get_metadata, mcp__figma__get_screenshot, mcp__figma__get_design_context, mcp__figma__get_variable_defs, mcp__figma__get_code_connect_map
---

너는 지금부터 **figma-implementer 프로세스를 메인 루프에서 직접 수행**한다.
대상: `$ARGUMENTS`

아래 4단계를 **순서대로** 거친다. 각 단계는 카파시 원칙 하나를 강제하며, 한 단계라도 실패하면 **멈춘다.**
서브에이전트에 위임하지 말고 이 절차를 직접 실행한다.

## 1. Clarify (원칙 1: Think Before Coding)

- Figma 링크/노드가 **없거나 모호하면 멈추고 질문한다.** 추측으로 진행 금지.
- 확인할 것: 어떤 프레임/컴포넌트인지, 포함할 변형(variant/state), 출력 파일 경로, **측정 가능한 완료 조건**.
...
- 같은 폴더에 `.stories.tsx` 를 추가하고 `parameters.design.url` 로 Figma 노드를 연결한다.
